...
from utils.XAI_utils import calculate_attributions, get_attrs_atoms, calculate_XAI_metrics, create_XAI_report, get_smarts_mols, find_hot_spots, find_substructures
...
def run_XAI(opt):
    ...
    train_loader = DataLoader(torch.load("{}/train_loader.pth".format(exp_dir)).dataset.shuffle()[:10])
    test_loader = DataLoader(torch.load("{}/test_loader.pth".format(exp_dir)).dataset.shuffle())
    ...
    if opt.XAI_mode == 'evaluate':
        ...
        for threshold in range(0, 100, 5):
            accuracies_train = []
            mol_attrs_train = {key: mol_attrs_train[key] for key in train_smiles if key in mol_attrs_train}
            results_train = calculate_XAI_metrics(opt, mol_attrs=mol_attrs_train, mol_smiles= train_smiles, threshold=threshold/100, save_results=False)
        ...
        results_test = calculate_XAI_metrics(opt, mol_attrs=mol_attrs, mol_smiles=test_smiles, threshold=best_threshold, save_results=True, logdir=f'{log_dir}')

        create_XAI_report(opt, best_threshold, threshold_results, results_test, log_dir)

    elif opt.XAI_mode == 'get_importance':
        find_hot_spots(opt, mol_attrs, test_smiles, threshold=0.5, save_results=True, logdir=f'{log_dir}/find_hot_spots')
        find_substructures(opt, mol_attrs, test_smiles, threshold=0.5, save_results=True, logdir=f'{log_dir}/find_patterns')
